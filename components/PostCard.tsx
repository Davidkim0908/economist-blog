...
export default function PostCard({ post, className, variant = 'default' }: PostCardProps) {
  ...
  if (variant === 'horizontal') {
    return (
      <Link href={`/posts/${post.category}/${post.slug}`} className={cn("group block", className)}>
        <div className="flex flex-col md:flex-row gap-8 items-center bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm transition-all duration-500 hover:shadow-xl hover:border-primary/10 hover:-translate-y-1 h-full">
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center">
            ...
            <h3 className="text-2xl md:text-3xl font-serif font-black mb-4 group-hover:text-primary transition-colors leading-tight tracking-tighter text-gray-900">
              {post.title}
            </h3>
            <p className="text-gray-600 text-base leading-relaxed line-clamp-3 mb-6 font-light">
              {post.excerpt}
            </p>
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-900 border-b-2 border-gray-900 self-start pb-1 group-hover:text-primary group-hover:border-primary transition-colors">
              Read Story
            </span>
          </div>
        </div>
  ...
  return (
    <Link href={`/posts/${post.category}/${post.slug}`} className={cn("group block h-full", className)}>
      <div className="flex flex-col h-full bg-white p-5 rounded-[2rem] border border-transparent transition-all duration-500 hover:border-gray-100 hover:shadow-lg hover:-translate-y-1">
        ...
        <h3 className="text-xl font-serif font-black mb-3 group-hover:text-primary transition-colors leading-tight tracking-tighter text-gray-900 px-1">
          {post.title}
        </h3>
        <p className="text-gray-600 text-sm leading-relaxed line-clamp-3 font-light px-1">
          {post.excerpt}
        </p>
      </div>
    </Link>
